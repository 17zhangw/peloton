...
// ===================================================================
template <class Node, class OperatorType, class OperatorExpr>
std::set<GroupID> RewriteTask<Node,OperatorType,OperatorExpr>::GetUniqueChildGroupIDs() {
...
template <class Node, class OperatorType, class OperatorExpr>
bool RewriteTask<Node,OperatorType,OperatorExpr>::OptimizeCurrentGroup(bool replace_on_match) {
  std::vector<RuleWithPromise<Node,OperatorType,OperatorExpr>> valid_rules;
      ...
      while (iterator.HasNext()) {
        ...
        if (!after.empty()) {
          ...
          if (replace_on_match) {
            ...
            std::shared_ptr<GroupExpression<Node,OperatorType,OperatorExpr>> new_gexpr;
...
void TopDownRewrite<Node,OperatorType,OperatorExpr>::execute() {
  ...
  did_optimize = this->OptimizeCurrentGroup(replace_on_transform_);
  ...
  if (did_optimize && replace_on_transform_) {
    auto top = new TopDownRewrite<Node,OperatorType,OperatorExpr>(this->group_id_, this->context_, this->rule_set_name_);
  ...
  for (auto g_id : child_groups) {
    auto top = new TopDownRewrite<Node,OperatorType,OperatorExpr>(g_id, this->context_, this->rule_set_name_);
    top->SetReplaceOnTransform(replace_on_transform_);
    this->PushTask(top);
  }
}
...
void BottomUpRewrite<Node,OperatorType,OperatorExpr>::execute() {
  if (!has_optimized_child_) {
    this->PushTask(
      new BottomUpRewrite<Node,OperatorType,OperatorExpr>(
        this->group_id_, this->context_, this->rule_set_name_, true
      )
    );
    ...
    for (auto g_id : child_groups) {
      this->PushTask(
        new BottomUpRewrite<Node,OperatorType,OperatorExpr>(
          g_id, this->context_, this->rule_set_name_, false
        )
      );
    }
  ...
  if (this->OptimizeCurrentGroup(true)) {
    this->PushTask(
      new BottomUpRewrite<Node,OperatorType,OperatorExpr>(
        this->group_id_, this->context_, this->rule_set_name_, false
      )
    );
  }
}
